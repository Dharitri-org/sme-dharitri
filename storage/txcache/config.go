...
const minGasPriceNanoMoaLowerBound = 1
...
func (config *ConfigSourceMe) verify() error {
	...
	if config.MinGasPriceNanoMoa < minGasPriceNanoMoaLowerBound {
		return fmt.Errorf("%w: config.MinGasPriceNanoMoa is invalid", storage.ErrInvalidConfig)
	}
