...
func (sc *scProcessor) createSCRForSender(
	...
	refundMoa := big.NewInt(0)
	refundMoa.Mul(big.NewInt(0).SetUint64(gasRemaining), big.NewInt(0).SetUint64(tx.GetGasPrice()))
	consumedFee.Sub(consumedFee, refundMoa)
	...
	if callType != vmcommon.AsynchronousCall {
		scTx.Value.Add(scTx.Value, refundMoa)
	}
...
func (sc *scProcessor) updateSmartContractCode(
	...
	// This check is desirable (not required though) since currently both Core and IELE send the code in the output account even for "regular" execution
