...
func (psh *PresenterStatusHandler) GetTotalRewardsValue() (string, string) {
	...
	rewardsInMoa := psh.computeRewardsInMoa()

	totalRewardsFloat := big.NewFloat(float64(numSignedBlocks))
	totalRewardsFloat.Mul(totalRewardsFloat, rewardsInMoa)
...
// CalculateRewardsPerHour will return an approximation of how many MOAs a validator will earn per hour
...
func (psh *PresenterStatusHandler) CalculateRewardsPerHour() string {
	...
	rewardsInMoa := psh.computeRewardsInMoa()
	if chanceToBeInConsensus == 0 || roundsPerHourAccordingToHitRate == 0 || rewardsInMoa.Cmp(big.NewFloat(0)) <= 0 {
		return "0"
	}
	...
	totalRewardsPerHourFloat.Mul(totalRewardsPerHourFloat, rewardsInMoa)
