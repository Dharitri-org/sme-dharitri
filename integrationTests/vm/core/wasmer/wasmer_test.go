...
func deploy(t *testing.T, wasmFilename string) (vmcommon.VMExecutionHandler, []byte) {
	...
	scCode := core.GetSCCode(wasmFilename)

	testContext := vm.CreatePreparedTxProcessorAndAccountsWithVMs(ownerNonce, ownerAddressBytes, ownerBalance)
	scAddressBytes, _ := testContext.BlockchainHook.NewAddress(ownerAddressBytes, ownerNonce, factory.CoreVirtualMachine)
	...
	wasmVM, _ := testContext.VMContainer.Get(factory.CoreVirtualMachine)
