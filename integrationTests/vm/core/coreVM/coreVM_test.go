...
var log = logger.GetOrCreate("coreVMtest")
...
func TestVmDeployWithTransferAndGasShouldDeploySCCode(t *testing.T) {
	...
	scCode := core.GetSCCode("../testdata/misc/fib_core.wasm")
	...
	tx := vm.CreateTx(
		...
		gasPrice,
		gasLimit,
		core.CreateDeployTxData(scCode),
	)
...
func TestSCMoveBalanceBeforeSCDeploy(t *testing.T) {
	...
	scAddressBytes, _ := testContext.BlockchainHook.NewAddress(ownerAddressBytes, ownerNonce+1, factory.CoreVirtualMachine)
...
func Benchmark_VmDeployWithFibbonacciAndExecute(b *testing.B) {
	runWASMVMBenchmark(b, "../testdata/misc/fib_core.wasm", b.N, 32, nil)
}

func Benchmark_VmDeployWithCPUCalculateAndExecute(b *testing.B) {
	runWASMVMBenchmark(b, "../testdata/misc/cpucalculate_core.wasm", b.N, 8000, nil)
}

func Benchmark_VmDeployWithStringConcatAndExecute(b *testing.B) {
	runWASMVMBenchmark(b, "../testdata/misc/stringconcat_core.wasm", b.N, 10000, nil)
}
...
func runWASMVMBenchmark(
	...
	tx := &transaction.Transaction{
		...
		GasPrice:  gasPrice,
		GasLimit:  gasLimit,
		Data:      []byte(core.CreateDeployTxData(scCode)),
		Signature: nil,
	}

	testContext := vm.CreateTxProcessorCoreVMWithGasSchedule(ownerNonce, ownerAddressBytes, ownerBalance, gasSchedule)
	defer testContext.Close()

	scAddress, _ := testContext.BlockchainHook.NewAddress(ownerAddressBytes, ownerNonce, factory.CoreVirtualMachine)
...
func TestGasModel(t *testing.T) {
	...
	runWASMVMBenchmark(t, "../testdata/misc/fib_core.wasm", 1, 32, gasSchedule)
	fmt.Println("CPUCALCULATE 8000 ")
	runWASMVMBenchmark(t, "../testdata/misc/cpucalculate_core.wasm", 1, 8000, gasSchedule)
	fmt.Println("STRINGCONCAT 1000 ")
	runWASMVMBenchmark(t, "../testdata/misc/stringconcat_core.wasm", 1, 10000, gasSchedule)
...
func TestWASMNamespacing(t *testing.T) {
	...
	// by Core, then this SC should have no problem to call imported functions
	// (as if it were run by Ethereuem).
	scCode := core.GetSCCode("../testdata/misc/fib_ewasmified.wasm")
...
func TestWASMMetering(t *testing.T) {
	...
	scCode := core.GetSCCode("../testdata/misc/cpucalculate_core.wasm")
...
func deployAndExecuteERC20WithBigInt(t *testing.T, numRun int, gasSchedule map[string]map[string]uint64) {
	...
	scCode := core.GetSCCode("../testdata/erc20-c-03/wrc20_core.wasm")
	...
	tx := vm.CreateDeployTx(
		...
		gasPrice,
		gasLimit,
		core.CreateDeployTxData(scCode)+"@"+initialSupply,
	)
...
func TestJournalizingAndTimeToProcessChange(t *testing.T) {
	...
	testContext := vm.CreateTxProcessorCoreVMWithGasSchedule(ownerNonce, ownerAddressBytes, ownerBalance, nil)
	defer testContext.Close()
	...
	tx := vm.CreateDeployTx(
		...
		gasPrice,
		gasLimit,
		core.CreateDeployTxData(scCode)+"@00"+hex.EncodeToString(ownerBalance.Bytes()),
	)
