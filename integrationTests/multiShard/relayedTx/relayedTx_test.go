...
import (
	...
	vmFactory "github.com/Dharitri-org/sme-dharitri/process/factory"
...
func TestRelayedTransactionInMultiShardEnvironmentWithSmartContractTX(t *testing.T) {
	...
	scCode := core.GetSCCode("../../vm/core/testdata/erc20-c-03/wrc20_core.wasm")
	scAddress, _ := ownerNode.BlockchainHook.NewAddress(ownerNode.OwnAccount.Address, ownerNode.OwnAccount.Nonce, vmFactory.CoreVirtualMachine)
	...
	integrationTests.CreateAndSendTransactionWithGasLimit(
		...
		make([]byte, 32),
		[]byte(core.CreateDeployTxData(scCode)+"@"+initialSupply),
...
func TestRelayedTransactionInMultiShardEnvironmentWithDCTTX(t *testing.T) {
	if testing.Short() {
		t.Skip("this is not a short test")
	}
	...
	txData = core.BuiltInFunctionDCTTransfer + "@" + hex.EncodeToString([]byte(tokenName)) + "@" + hex.EncodeToString(valueToTopUp.Bytes())
	for _, player := range players {
	...
	txData = core.BuiltInFunctionDCTTransfer + "@" + hex.EncodeToString([]byte(tokenName)) + "@" + hex.EncodeToString(sendValue.Bytes())
	transferTokenDCTGas := uint64(1)
	transferTokenBaseGas := tokenIssuer.EconomicsData.ComputeGasLimit(&transaction.Transaction{Data: []byte(txData)})
	transferTokenFullGas := transferTokenBaseGas + transferTokenDCTGas
	nrRoundsToTest := int64(5)
	for i := int64(0); i < nrRoundsToTest; i++ {
		for _, player := range players {
	...
	finalBalance := big.NewInt(0).Mul(big.NewInt(int64(len(players))), big.NewInt(nrRoundsToTest))
	finalBalance.Mul(finalBalance, sendValue)
	checkAddressHasDCTTokens(t, receiverAddress1, nodes, tokenName, finalBalance)
	checkAddressHasDCTTokens(t, receiverAddress2, nodes, tokenName, finalBalance)
